...
def get_data_loaders(base_path, categories, batch_size=16):
    ...
    transform = transforms.Compose([
        transforms.Resize((224, 224)),  # Resize images to fit the model's expected input
...
def train_and_evaluate(base_path, categories, num_epochs=15, batch_size=16):
    # Setup device
    ...
    # Initialize DenseNet201 with pretrained weights
    densenet201 = models.densenet201(pretrained=True)
    num_features = densenet201.classifier.in_features
    densenet201.classifier = nn.Linear(num_features, len(categories))
    densenet201 = densenet201.to(device)

    # Unfreeze more layers for fine-tuning
    for param in densenet201.features.parameters():
        param.requires_grad = False
    for param in list(densenet201.features.children())[-4:]:
    ...
    optimizer = optim.Adam(filter(lambda p: p.requires_grad, densenet201.parameters()), lr=0.001)
    ...
    for epoch in range(num_epochs):
        ...
        for inputs, labels in tqdm(train_loader, desc=f"Training Epoch {epoch+1}/{num_epochs}", leave=False):
            ...
            loss.backward()
            nn.utils.clip_grad_norm_(densenet201.parameters(), max_norm=2.0)  # Gradient clipping
            optimizer.step()
        ...
        # Validation phase
        val_loss, val_accuracy = evaluate_model(densenet201, val_loader, criterion, device)
        ...
        if val_loss < best_val_loss:
            ...
            torch.save(densenet201.state_dict(), 'best_densenet_model.pth')
    ...
    densenet201.load_state_dict(torch.load('best_densenet_model.pth'))
    evaluate_model(densenet201, val_loader, criterion, device, final_report=True, categories=categories)
